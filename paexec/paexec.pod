...
Command and its parameters are specified by free arguments.
...
Set alternative string for 'success' message.

=item B<-m> f=I<failure>

Set alternative string for 'failure' message.

=item B<-m> F=I<fatal>

Set alternative string for 'fatal' message.
An empty string for 'fatal'
means it will not be output to stdout in case of fatal error.

=item B<-m> t=I<eot>

Set alternative string for EOT message.

=item B<-m> d=I<delimiter>

Set alternative string for tasks delimiter (for I<-g> mode).
...
Change paexec to expect NUL characters as separators,
instead of newlines.  This is expected to be used in
